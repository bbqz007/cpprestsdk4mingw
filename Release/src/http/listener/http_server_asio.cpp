...
will_deref_and_erase_t asio_server_connection::async_write(WriteFunc response_func_ptr, const http_response& response)
{
    if (m_ssl_stream)
    {
        boost::asio::async_write(*m_ssl_stream, m_response_buf, [=](const boost::system::error_code& ec, std::size_t wrsize) {
#if defined(__MINGW32__) && defined(Z_PORT_MINGW)
            // resolve boost 1.5x problem
            if (!ec)
            {
                if(wrsize == 0)
    ...
    {
        boost::asio::async_write(*m_socket, m_response_buf, [=](const boost::system::error_code& ec, std::size_t wrsize) {
#if defined(__MINGW32__) && defined(Z_PORT_MINGW)
            if (!ec)
            {
                ucout << "sent " << wrsize << " bytes\n";
                if(wrsize == 0)
                    m_response_buf.consume(m_response_buf.size());
            }
#endif
