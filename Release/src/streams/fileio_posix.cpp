...
#include <io.h>
#include <fcntl.h>
#define pread(a,b,c,d) ((lseek(a,d,SEEK_SET) != -1)? read(a,b,c): -1)
#define pwrite(a,b,c,d) ((lseek(a,d,SEEK_SET) != -1)? write(a,b,c): -1)
#if defined(_UNICODE)
#include <locale>
#include <codecvt>
#endif //_UNICODE
...
bool _open_fsb_str(_filestream_callback* callback, 
                   ...
                   const char* filename, 
#endif
                   std::ios_base::openmode mode, int prot)
{
    if (callback == nullptr || filename == nullptr) return false;
#if defined(__MINGW32__) && defined(Z_PORT_MINGW) && defined(_UNICODE)
    std::wstring string_to_convert(filename);
    ...
    std::string name = converter.to_bytes( string_to_convert );
#else
